...
2. **EL VEREDICTO SE DERIVA DEL ANÁLISIS, CON ASIMETRÍA Y ACEPTACIÓN HUMANA AUTENTICADA CRIPTOGRÁFICAMENTE (NO FALSIFICABLE).** Quien formula una propuesta no dicta su propio resultado. El veredicto se deriva mecánicamente del análisis de impacto, la competencia y el estrés de mitigaciones. El operador humano responsable puede **endurecer** el veredicto o registrar formalmente una aceptación de riesgo (`HUMAN_RISK_ACCEPTED`) mediante acción autenticada fuera de banda (`accept-risk` con clave privada pre-enrolada, justificación causal $\ge 25$ caracteres, firma asimétrica **Ed25519**, verificación contra el registro confiable de autoridades `policies/authorities.json`, vinculación criptográfica al **`proposalDigest` exacto de 64 caracteres**, protección anti-symlink TOCTOU atómica, alcance por entorno y caducidad TTL). Se prohíbe terminantemente que el agente **auto-declare o falsifique** la aceptación de riesgo dentro del payload de la propuesta (rechazado fail-closed con `UNAUTHENTICATED_HUMAN_OVERRIDE`); claves no registradas son rechazadas con `UNTRUSTED_KEY_ID`; firmas manipuladas con `INVALID_ED25519_SIGNATURE`; digests truncados o discrepantes con `INVALID_PROPOSAL_DIGEST_FORMAT` o `PROPOSAL_DIGEST_MISMATCH`; y carreras de enlace con `SYMLINK_DETECTED`.
...
1. **Raíz de Confianza en Registro de Autoridades**: La clave pública nunca se extrae del envelope del archivo; se valida contra el registro soberano (`policies/authorities.json` o `.axion/keys/authorities.json`). El actor debe contar con rol `HUMAN_AUTHORITY` y estado `TRUSTED`. Si un agente genera su propio par de claves en memoria, es rechazado de inmediato con `UNTRUSTED_KEY_ID` (exit 1).
2. **Cero Generación de Claves al Vuelo**: `accept-risk` no genera claves efímeras ni fallbacks locales. La clave privada debe corresponder a una identidad humana previamente enrolada y custodiada fuera de banda.
3. **Vinculación Criptográfica de 64 Caracteres**: El `proposalDigest` debe ser un hash SHA-256 completo de 64 caracteres hexadecimales (`/^[a-f0-9]{64}$/`) que coincida bit a bit (`===`) con el digest canónico de la propuesta. Prefijos parciales o truncados (ej. 16, 32 o 63 caracteres) son rechazados fail-closed con `INVALID_PROPOSAL_DIGEST_FORMAT` (exit 1).
4. **Protección Anti-Symlink y TOCTOU**: Creación atómica exclusiva (`O_CREAT | O_EXCL` con permisos `0o600`), validación post-renombrado y doble verificación pre/post-lectura de inodo y mtime para eliminar ventanas de carrera de sustitución concurrente de symlinks.
5. **Control de Entorno (Scoping)**: Si el entorno de ejecución actual es `production` pero la aceptación solo autoriza `development`, el motor bloquea el avance con veredicto `ENVIRONMENT_MISMATCH` (exit 2).
6. **Caducidad (TTL)**: Vencido el plazo (`expiresAt`), la excepción queda revocada automáticamente (`RISK_ACCEPTANCE_EXPIRED`, exit 1).
