...
// Antigravity monta las skills desde .agents/skills/<nombre>/SKILL.md; el directorio
// plano de workflows quedó obsoleto y por eso el protocolo no arrancaba solo en sus chats.
const DIR_SK = path.join(ROOT, '.agents', 'skills');
const rutaSkill = (wf) => path.join(DIR_SK, wf.replace(/\.md$/, ''), 'SKILL.md');
const DIR_CC = path.join(ROOT, '.claude', 'commands');

// Derivada del disco: una copia escrita a mano solo comprueba los comandos que alguien
// se acordó de añadirle, y el que se olvide queda sin contrato que lo vigile.
const WORKFLOWS = fs.readdirSync(DIR_SK, { withFileTypes: true })
...
// --- 1. Todos los comandos, en las dos superficies que los agentes leen ---
for (const wf of WORKFLOWS) {
  ok(fs.existsSync(rutaSkill(wf)), `falta .agents/skills/${wf.replace(/.md$/, "")}/SKILL.md (Antigravity)`);
  ok(fs.existsSync(path.join(DIR_CC, wf)), `falta .claude/commands/${wf} (Claude Code)`);
}
console.log(`✓ ${WORKFLOWS.length} skills presentes en .agents/skills y en .claude/commands`);
...
for (const wf of WORKFLOWS) {
  ok(leer(rutaSkill(wf)) === leer(path.join(DIR_CC, wf)),
...
for (const wf of WORKFLOWS) {
  for (const m of leer(rutaSkill(wf)).matchAll(/node\s+(tools\/[a-z0-9_]+\.js)/gi)) {
