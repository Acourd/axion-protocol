...
1. **Anclaje Criptográfico de la Raíz de Confianza**: La clave pública nunca se extrae del envelope del archivo; se valida contra el registro soberano (`policies/authorities.json`) el cual está sellado criptográficamente por la clave raíz de gobernanza (`policies/authorities.seal.json`). Cualquier alteración no autorizada del registro es detectada y rechazada fail-closed (`UNTRUSTED_REGISTRY_MODIFICATION`, exit 1).
2. **Custodia Estricta de Claves Privadas Fuera del Workspace**: Las claves privadas de operadores humanos jamás residen dentro del repositorio/workspace del agente (`WORKSPACE_PRIVATE_KEY_FORBIDDEN`). Prohibida la invocación desde contextos de agentes (`AGENT_INVOCATION_FORBIDDEN`). La aceptación para entornos de producción está bloqueada localmente (`PRODUCTION_RISK_ACCEPTANCE_FORBIDDEN`).
...
4. **Inmutabilidad y Libro Append-Only**: Prohibida la sobrescritura silenciosa de registros (`ACCEPTANCE_ALREADY_EXISTS`). Todo evento se registra en el libro append-only `risk-acceptance-ledger.jsonl` y se archiva en copias históricas por timestamp.
5. **Revocación Formal Firmada**: La revocación formal (`revokeRisk`) invalida de inmediato cualquier autorización previa (`RISK_ACCEPTANCE_REVOKED`, exit 1).
