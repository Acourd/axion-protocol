...
2. **EL VEREDICTO SE DERIVA DEL ANÁLISIS, CON ASIMETRÍA Y ACEPTACIÓN HUMANA AUTENTICADA CRIPTOGRÁFICAMENTE (NO FALSIFICABLE).** Quien formula una propuesta no dicta su propio resultado. El veredicto se deriva mecánicamente del análisis de impacto, la competencia y el estrés de mitigaciones. El operador humano responsable puede **endurecer** el veredicto o registrar formalmente una aceptación de riesgo (`HUMAN_RISK_ACCEPTED`) mediante acción autenticada fuera de banda (`accept-risk` con operador, justificación causal $\ge 25$ caracteres, firma asimétrica **Ed25519**, vinculación criptográfica al **`proposalDigest` exacto**, alcance por entorno y caducidad TTL). Se prohíbe terminantemente que el agente **auto-declare o falsifique** la aceptación de riesgo dentro del payload de la propuesta (rechazado fail-closed con `UNAUTHENTICATED_HUMAN_OVERRIDE`); registros manipulados o con firma inválida son rechazados con `INVALID_ED25519_SIGNATURE`; y cualquier mutación posterior a la propuesta invalida la autorización con `PROPOSAL_DIGEST_MISMATCH`.
