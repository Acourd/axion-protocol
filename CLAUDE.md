...
6. **Autonomous Drive Execution:** When invoked with `/drive`, execute autonomously in continuous closed loop without micro-interruptions. Classify complexity (Fast-Loop vs Deep-Loop), use checkpoints before multi-file edits, verify deterministically with exit code 0 (`node tests/run_all.js` or `node tools/verify_changes.js`), and conclude with the 3-line executive report (`✓ [Acción Cumplida]`, `📊 [Métricas]`, `🧠 [Próximo Vector Metacognitivo]`).
